...
为了验证这个动手尝试项目，你只需要提交一个经过训练的模型。**没有达到最低结果要求来验证他**。
...
我们将撰写一篇博文详细介绍这个 AI vs. AI 工具，但为了给你一个大致的概念，他的工作原理如下：
...
⚠ 我们将使用一个实验版本的 ML-Agents，他允许你将模型从 Hub 中上传和下载到 / 。** 你需要安装相同的版本。**
...
该环境称为 *SoccerTwos*。这是由 Unity MLAgents 团队制作的。你可以在[这里](https://github.com/Unity-Technologies/ml-agents/blob/develop/docs/Learning-Environment-Examples.md#soccer-twos)找到他的文档。
...
这个想法简单而强大：一个集中化的评论家**处理团队中所有智能体程序的状态，以估计每个智能体程序的表现如何**。可以将这个评论家看作是一名教练。
...
有多个超参数。要更好地了解他们，你应该查看**[文档](https://github.com/Unity-Technologies/ml-agents/blob/release_20_docs/docs/Training-Configuration-File.md)**中的每个解释。

我们要在这里使用的配置文件位于 `./config/poca/SoccerTwos.yaml` ，他的内容如下：
...
与 Pyramids 或 SnowballTarget 相比，我们有了一个包含自我对抗部分的新超参数。你如何修改他们对于获得良好结果至关重要。
...
现在我们已经训练好了我们的智能体，我们准备将他们推送到 Hub 上，以便参加 AI 对抗赛并在浏览器上观看他们的比赛🔥。
...
3. `-repo-id`：你想要创建或更新的Hugging Face repo的名称。他的格式是<你的Huggingface用户名>/<repo名称>。如果该repo不存在，**他将被自动创建**。
...
这是指向你模型的链接。他包含一个模型卡片，其中解释了如何使用模型，你的 Tensorboard 和配置文件。**令人惊奇的是，这是一个 git 存储库，这意味着你可以拥有不同的提交，通过新的推送更新你的存储库等等。**

## 步骤 6：验证你的模型是否准备参加 AI vs AI 挑战

既然你的模型已推送到Hub，**他将自动添加到 AI vs AI 挑战模型池中。**在你的模型被添加到排行榜之前可能需要一些时间，因为我们每 4 小时进行一次比赛。
...
我们强烈建议在将模型推送到 Hub 时创建一个新的模型，如果你想要重新训练他或训练一个新版本。

## 步骤7：在我们的演示中可视化一些对战

现在，你的模型已经成为 AI vs AI 挑战赛的一部分，**你可以通过以下链接可视化他与其他模型的对战结果**：https://huggingface.co/spaces/unity/ML-Agents-SoccerTwos
...
请注意，这些实时观看的比赛结果不会用于计算你的最终结果，他们仅用于可视化你的智能体性能。
