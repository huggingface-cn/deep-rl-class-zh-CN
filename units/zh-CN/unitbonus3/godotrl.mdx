...
我们今天要构建的环境称为 Ring Pong，他是乒乓球游戏，但球场是一个环形，球拍在环上移动。**目标是让球在环内持续弹跳**。
...
Godot 引擎是一个功能齐全的跨平台游戏引擎，旨在通过统一的界面创建 2D 和 3D 游戏。他提供了一套全面的常用工具，使用户可以专注于制作游戏，而无需重新造轮子。游戏可以一键导出到多个平台，包括主要的桌面平台（Linux、macOS、Windows），以及移动（Android、iOS）和基于 Web 的（HTML5）平台。
...
右键点击 **Player** 节点，然后点击 **Add Child Node** 。这里列出了许多节点，搜索 `AIController3D` 并创建他。
...
AI 控制器节点应该已经添加到场景树中，在其旁边有一个滚动条。点击他以打开附加到 AI 控制器的脚本。Godot 游戏引擎使用一种名为 GDScript 的脚本语言，他在语法上类似于 Python。该脚本包含需要实现的方法，以使我们的 AI 控制器正常工作。
...
你可以在编辑器中实时运行训练，首先使用 `gdrl` 启动 Python 训练。
