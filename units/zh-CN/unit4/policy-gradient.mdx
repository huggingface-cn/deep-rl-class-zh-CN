...
一个想法是让**智能体在一个完整的片段中进行交互**。如果我们在这个片段获胜，我们就认为每个采取的动作都是好的，将来必须更多地采样他们，因为他们导致了胜利。
...
**但是我们怎么知道我们的政策是否好呢？**我们需要有一种方法来衡量他。要知道这一点，我们定义了一个分数/目标函数，称为 \\(J(\theta)\\)。
...
- \\(R(\tau)\\) :  这是一条任意轨迹的回报。为了使用他来计算期望回报，我们需要将其乘以每个可能轨迹的概率。
- \\(P(\tau;\theta)\\) : 每个可能轨迹 \\(\tau\\) 的概率（该概率取决于 \\(\theta\\)，因为他定义了用于选择轨迹动作的策略，这影响了所访问的状态）。
...
- \\(J(\theta)\\) : 期望回报，我们通过对所有轨迹进行求和来计算他，其中包括给定 \\(\theta \\) 下采取该轨迹的概率以及该轨迹的回报。
...
策略梯度是一个优化问题：我们想要找到最大化我们的目标函数 \\(J(\theta)\\) 的 \\(\theta\\) 值，因此我们需要使用 **梯度上升**。他是 *梯度下降* 的反向，因为他给出了 \\(J(\theta)\\) 最陡增加的方向。

（如果你需要关于梯度下降和梯度上升之间的区别的提醒，请参阅[这个](https://www.baeldung.com/cs/gradient-descent-vs-ascent)和[这个](https://stats.stackexchange.com/questions/258721/gradient-ascent-vs-gradient-descent-in-logistic-regression)）
...
2. 我们还有另一个问题，我在下一个可选章节中详细介绍。为了对这个目标函数求导，我们需要对状态分布进行求导，这被称为马尔可夫决策过程动力学，他与环境相关。他给出了环境在给定当前状态和智能体采取的动作的情况下进入下一个状态的概率。问题是我们可能不知道他，因此无法对他进行求导。

<img src="https://huggingface.co/datasets/huggingface-deep-rl-course/course-images/resolve/main/en/unit6/probability.png" alt="Probability"/>

幸运的是，我们将使用一种称为策略梯度定理的解决方案，他将帮助我们将目标函数重新表述为一个不涉及状态分布微分的可微分函数。
...
强化算法, 也叫做蒙特卡洛策略梯度, 是一种策略梯度算法，**他使用来自整个回合的估计回报来更新策略参数 \\(\theta\\)**：
...
我们可以做出以下解释：
- \\(\nabla_\theta log \pi_\theta(a_t|s_t)\\) 是选择从状态 \\(s_t\\) 选取动作 \\(a_t\\) 的 (log) 概率的**最陡升高方向**。他告诉我们：如果我们想要增加/减少在状态 \\(s_t\\) 选取动作 \\(a_t\\) 的对数概率，应该如何改变策略的权重。
- \\(R(\tau)\\) 是评分函数：
  - 如果回报很高，他将**推高**（状态，动作）组合的概率。
  - 反之，如果回报很低，他将**降低**（状态，动作）组合的概率。
