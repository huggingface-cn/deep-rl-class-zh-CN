...
现在我们已经学习了强化算法背后的理论，**你已经准备好用 Pytorch 编写你的强化算法智能体了**。并且你会用 CartPole-v1 和 PixelCopter 来测试他的鲁棒性。
...
# 单元 4: 用 Pytorch 编写你的第一个深度强化学习算法：强化算法，并测试他的鲁棒性 💪

<img src="https://huggingface.co/datasets/huggingface-deep-rl-course/course-images/resolve/main/en/unit6/thumbnail.png" alt="thumbnail"/>


在本 notebook 中, 你将从头开始编写你的第一个深度强化学习算法：强化算法（也称为蒙特卡罗策略梯度）。
...
最好将这个 colab 复制到你的 Google Drive 中运行，这样如果**他超时了**，你仍然可以在你的 Google Drive 中保存已保存的 Notebook ，而不需要重新开始。
...
- `huggingface_hub`: Hub 是一个中心化的平台，任何人都可以在其中分享和探索模型和数据集。他提供版本控制、度量、可视化等功能，使你能够轻松地与他人合作。

你可能想知道为什么我们安装的是 gym 而不是 gymnasium，一个更新版本的 gym ？因为我们使用的 gym-games 还没有更新到 gymnasium。

你会遇到的不同之处有：

在 `gym` 中，我们没有 `terminated` 和 `truncated` ，只有 `done` 。
在 `gym` 中，使用 `env.step()` 返回的是 `state, reward, done, info`
...
## 创建 CartPole 环境并理解他怎么运行的

### [环境 🎮](https://www.gymlibrary.dev/environments/classic_control/cart_pole/)

### 为什么使用像 CartPole-v1 这样简单的环境?

如 [强化学习技巧和技巧](https://stable-baselines3.readthedocs.io/en/master/guide/rl_tips.html) 中所述，当你从头开始实现你的智能体时，你需要确保他**在简单环境中可以正确工作并找到易于出错的地方，然后再深入进行**。因为在简单环境中发现错误将更容易。
...
- 这意味着 `m.log_prob(action)` 中的 `action` 必须是一个 Tensor **但他不是。**
...
- 由于**我们想要从动作的概率分布中采用一个动作**，我们就不能使用  `action = np.argmax(m)` 因为他总会输出概率最高的动作。
...
- 为什么？因为我们的策略应该只基于后果强化行动：因此，在执行行动之前获得的奖励是无用的（因为他们不是因为行动而获得的），**只有在行动之后获得的奖励才是有意义的**。
...
奖励函数 💰:
- 每穿过一个垂直的方块，他获得一个 +1 的正奖励。每当到达终止状态时，他会获得一个 -1 的负奖励。
