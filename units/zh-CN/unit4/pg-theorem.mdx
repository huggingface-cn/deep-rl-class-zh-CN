...
然后我们将总和中的每一项乘以 \\(\frac{P(\tau;\theta)}{P(\tau;\theta)}\\（这是可能的，因为他 = 1）
...
接着我们可以运用“对数梯度技巧”（也称“似然比技巧”或“REINFORCE技巧”），这是微积分中的一个简单规则，他意味着 \\( \nabla_x log f(x) = \frac{\nabla_x f(x)}{f(x)} \\)
...
由于 MDP 的初始状态分布或状态转换动力学都不依赖于 \\(\theta\\) ，两项的导数都是 0。因此我们可以删除他们：
