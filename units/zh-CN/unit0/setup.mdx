# 基础配置

我们要做两件事：

1. **创建 Hugging Face 帐户**（如果尚未完成）
2. **注册 Discord 并自我介绍**（不要害羞🤗）

### 创建 Hugging Face 帐户

（如果尚未完成）在 <a href="https://huggingface.co/join">此处</a> 创建一个 HF 帐户

### 让我们加入我们的 Discord 服务器

你现在可以注册我们的 Discord 服务器。 在这里你**可以与社区和我们交流，创建和加入学习小组以相互成长等等**

👉🏻在<a href="https://discord.gg/ydHrjt3WP5">此处</a>加入我们的discord服务器。

加入时，记得在#introduce-yourself 中介绍自己，并在#role-assignments 中注册强化学习频道。

我们有多个 RL 相关的频道：

- `rl-announcements`：我们在这里提供有关课程的最新信息。
- `rl-discussions`：你可以在这里交流 RL 和共享信息。
- `rl-study-group`：你可以在其中创建和加入学习小组。
- `rl-i-made-this`：你可以在这里分享你的项目和模型。

如果这是你第一次使用 Discord，我们编写了文档 **Discord 101** 以帮助你获得最佳实践。

恭喜！ **你刚刚完成基础配置**。 你现在可以开始学习深度强化学习了。 玩得开心！


### 保持热爱，奔赴山海🤗