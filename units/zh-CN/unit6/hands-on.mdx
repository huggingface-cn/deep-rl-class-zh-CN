# 利用 PyBullet 和 Panda-Gym 进行机器人模拟的 **优势演员--评论员算法** (A2C) 🤖[[hands-on]]
...
现在您已经研究了 **优势演员--评论员算法** (A2C) 背后的理论，**您已准备好在机器人环境中使用 Stable-Baselines3 训练您的 A2C 智能体**。 并训练两个机器人：
...
要验证认证过程的实际操作，您需要将两个经过训练的模型推送到 Hub 并获得以下结果：

- `AntBulletEnv-v0` 得到的结果需 >= 650.
- `PandaReachDense-v2` 得到的结果需 >= -3.5.

要找到您的结果，[转到排行榜](https://huggingface.co/spaces/huggingface-projects/Deep-Reinforcement-Learning-Leaderboard) 并找到您的模型，**结果 = 平均奖励 - 奖励标准* *
...
# 第 6 单元：利用 PyBullet 和 Panda-Gym 进行机器人模拟的 **优势演员--评论员算法** (A2C) 🤖
...
我们一直在努力改进我们的教程，所以 **如果您在此笔记本中发现一些问题**，请 [open an issue on the GitHub Repo](https://github.com/huggingface/deep-rl-class/issues).
...
- 能够**使用 A2C** 训练机器人。
- 理解为什么**我们需要规范化输入**。
- 能够**将您训练有素的智能体和代码推送到 Hub**，并附带有漂亮的视频回放和评估分数 🔥。
...
您可以在此处找到此环境的详细说明： https://hackmd.io/@jeffreymo/SJJrSJh5_#PyBullet
...
在强化学习中的一个较好的做法是 [normalize input features](https://stable-baselines3.readthedocs.io/en/master/guide/rl_tips.html).
...
在这种情况下，因为我们有一个包含 28 个值的向量作为输入，所以我们将使用 MLP（多层感知器）作为策略。
...
- 让我们用 2,000,000 个时间步长训练我们的代理，不要忘记在 Colab 上使用 GPU。 大约需要 25-40 分钟
...
通过使用 `package_to_hub`, 正如我们在之前的单元中提到的，**评估、录制回放、生成代理的模型卡并将其推送到Hub**。

通过如下方法：

- 您可以**展示我们的工作成果**🔥
- 你可以**可视化你的智能体过程** 👀
- 您可以**与社区共享其他人可以使用的智能体**💾
- 您可以**访问排行榜 🏆 以查看您的智能体与您的同学相比的表现**👉 https://huggingface.co/spaces/huggingface-projects/Deep-Reinforcement-Learning-Leaderboard
...
我们将使用此环境的密集版本。 这意味着我们将获得一个*密集的奖励函数*，**将在每个时间步长提供奖励**（智能体越接近完成任务，奖励越高）。 与*稀疏奖励函数*相反，其中环境**当且仅当任务完成时才返回奖励**。
...
鉴于它是一个作为观察的字典，**我们将需要使用 MultiInputPolicy 策略而不是 MlpPolicy**。
...
如果您想为 panda-gym 尝试更高级的任务，您需要检查使用 **TQC 或 SAC**（一种更适合机器人任务的样本效率更高的算法）完成了什么。 在真实的机器人技术中，出于一个简单的原因，您将使用样本效率更高的算法：与模拟过程相反**如果您将机械臂移动太多，则有损坏它的风险**。
